...
aladinAndroid.getAppBucketUrl = function(gaiaHubUrl, appPrivateKey) {
  aladin.getAppBucketUrl(gaiaHubUrl, appPrivateKey)
  .then(function(url) {
...
aladinAndroid.getUserAppFileUrl = function(path, username, appOrigin) {
    aladin.getUserAppFileUrl(path, username, appOrigin)
    .then(function(url) {
      if (url != null) {
...
aladinAndroid.lookupProfile = function(username, zoneLookupFileURL) {
  aladin.lookupProfile(username, zoneLookupFileURL)
  .then(function(userData) {
...
aladinAndroid.validateProofs = function(profile, ownerAddress, name) {
  aladin.validateProofs(JSON.parse(profile), ownerAddress, name)
...
aladinAndroid.extractProfile = function(token, publicKeyOrAddress) {
  return JSON.stringify(aladin.extractProfile(token, publicKeyOrAddress))
}

aladinAndroid.verifyProfileToken = function(token, publicKeyOrAddress) {
  return JSON.stringify(aladin.verifyProfileToken(token, publicKeyOrAddress))
}

aladinAndroid.signProfileToken = function(profile, privateKey, subject, issuer, signingAlgorithm, issuedAt, expiresAt) {
  return JSON.stringify(aladin.signProfileToken(JSON.parse(profile), privateKey, JSON.parse(subject), JSON.parse(issuer), signingAlgorithm, new Date(issuedAt), new Date(expiresAt)))
}

aladinAndroid.wrapProfileToken = function(token) {
  return JSON.stringify(aladin.wrapProfileToken(token))
}

aladinAndroid.parseZoneFile = function(zoneFileContent) {
  return JSON.stringify(parseZoneFile(zoneFileContent))
}


aladinAndroid.resolveZoneFileToProfile = function(zoneFileContent, publicKeyOrAddress) {
  aladin.resolveZoneFileToProfile(zoneFileContent, publicKeyOrAddress)
...
networkAndroid.getNamePrice = function(fullyQualifiedName) {
  aladin.config.network.getNamePrice(fullyQualifiedName).then(function(denomination){
...
networkAndroid.getNamespacePrice = function(namespaceId) {
  aladin.config.network.getNamespacePrice(namespaceId).then(function(denomination){
...
networkAndroid.getGracePeriod = function() {
  aladin.config.network.getGracePeriod().then(function(gracePeriod){
...
networkAndroid.getNamesOwned = function(address) {
  try {
      aladin.config.network.getNamesOwned(address).then(function(names){
...
networkAndroid.getNamespaceBurnAddress = function(namespaceId) {
  aladin.config.network.getNamespaceBurnAddress(namespaceId).then(function(address){
...
networkAndroid.getNameInfo = function(fullyQualifiedName) {
    aladin.config.network.getNameInfo(fullyQualifiedName).then(function(nameInfo) {
        try {
...
networkAndroid.getNamespaceInfo = function(namespaceId) {
  aladin.config.network.getNamespaceInfo(namespaceId).then(function(info){
...
networkAndroid.getZonefile = function(zonefileHash) {
  aladin.config.network.getZonefile(zonefileHash).then(function(content){
...
networkAndroid.getAccountStatus = function(address, tokenType) {
  aladin.config.network.getAccountStatus(address, tokenType).then(function(stati){
...
networkAndroid.getAccountHistoryPage = function(address, page) {
  aladin.config.network.getAccountHistoryPage(address, page).then(function(stati){
...
networkAndroid.getAccountAt = function(address, blockHeight) {
  aladin.config.network.getAccountAt(address, blockHeight).then(function(stati){
...
networkAndroid.getAccountTokens = function(address) {
   try {
      aladin.config.network.getAccountTokens(address).then(function(tokens) {
...
try {
  aladin.config.network.getAccountBalance(address, tokenType).then(function(balance){
...
networkAndroid.estimateTokenTransfer = function(recipientAddress, tokenType, tokenAmount, scratchArea, senderUtxos, additionalOutputs) {
  try {
    aladin.transactions.estimateTokenTransfer(recipientAddress, tokenType, Bigi(tokenAmount), scratchArea, senderUtxos, additionalOutputs).then(function(costs){
...
aladinAndroid.fetchResolve = function(key, response) {
  try {
    console.log("inside fetchResolve")
...
aladinAndroid.fetchReject = function(key, error) {
  try {

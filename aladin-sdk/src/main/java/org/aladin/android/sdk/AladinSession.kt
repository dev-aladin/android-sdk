...
import okhttp3.*
import org.aladin.android.sdk.j2v8.LogConsole
...
private val DEFAULT_ALADIN_HOST: String = "http://staging.aladinnetwork.org/auth"

private val DEFAULT_BETA_ALADIN_HOST: String = "http://staging.aladinnetwork.org/auth"

/**
 * Main object to interact with aladin in an activity or service
 *
 * The current implementation is a wrapper for aladin.js using a j2v8 javascript engine.
 *
 ...
 * @param config the configuration for aladin
 ...
 * @param betaMode flag indicating whether the beta version of the hosted aladin browser should be used. Defaults to false.
 */
class AladinSession(context: Context? = null, private val config: AladinConfig,
                    /**
                         * url of the name lookup service, defaults to
                         */
                        val nameLookupUrl: String = "",
    ...
    private val TAG = AladinSession::class.simpleName
    ...
    init {
        ...
        v8.executeVoidScript(scriptRepo.aladin())
        v8.executeVoidScript(scriptRepo.base64())
        v8.executeVoidScript(scriptRepo.zoneFile())
        v8.executeVoidScript(scriptRepo.aladinAndroid())


        v8aladinAndroid = v8.getObject("aladinAndroid")
        v8userSessionAndroid = v8.getObject("userSessionAndroid")
        v8networkAndroid = v8.getObject("networkAndroid")

        registerCryptoMethods()
        registerJSAndroidBridgeMethods(v8aladinAndroid, v8userSessionAndroid, callFactory)

        val scopesString = Scope.scopesArrayToJSONString(config.scopes)
        val authenticatorURL = if (betaMode) {
            DEFAULT_BETA_ALADIN_HOST
        } else {
            DEFAULT_ALADIN_HOST
        }
        v8.executeVoidScript("var appConfig = new aladin.AppConfig(${scopesString}, '${config.appDomain}', '${config.redirectPath}','${config.manifestPath}', null, '${authenticatorURL}');var userSession = new aladin.UserSession({appConfig:appConfig, sessionStore:androidSessionStore});")
    ...
    private fun registerJSAndroidBridgeMethods(v8aladinAndroid: V8Object, v8userSessionAndroid: V8Object, callFactory: Call.Factory) {
        val android = JSAndroidBridge(this, v8, v8aladinAndroid, v8userSessionAndroid, callFactory)
    ...
    /**
     * Generates an authentication request that can be sent to the aladin browser
     ...
     * sign with an existing aladin ID already on the device or create a new one.
     *
    ...
    /**
     * Redirects the user to the aladin browser to approve the sign in request given.
     *
     * The user is redirected to the aladinIDHost if the aladin: protocol handler is not detected.
     ...
     * @param aladinIDHost    the URL to redirect the user to if the aladin protocol handler is not detected
     */
    fun redirectToSignInWithAuthRequest(authRequest: String, aladinIDHost: String = if (betaMode) DEFAULT_BETA_ALADIN_HOST else DEFAULT_ALADIN_HOST,
     ...
     * @param zoneFileLookupURL the url of the zone file lookup service
    ...
    fun lookupProfile(username: String, zoneFileLookupURL: URL = URL(nameLookupUrl), callback: (Result<Profile>) -> Unit) {
        ...
        v8aladinAndroid.executeVoidFunction("lookupProfile", v8params)
        v8params.release()
    }
     ...
     * @param name (default = null) The aladin name to be validated
    ...
    fun validateProofs(profile: Profile, ownerAdress: String, name: String? = null, callback: (Result<ArrayList<Proof>>) -> Unit) {
        ...
        v8aladinAndroid.executeVoidFunction("validateProofs", params)
        params.release()
    }
    ...
    fun extractProfile(token: String, publicKeyOrAddress: String? = null): ProfileToken {
        ...
        try {
            val decodedToken = v8aladinAndroid.executeStringFunction("extractProfile", params)
    ...
    fun wrapProfileToken(token: String): ProfileTokenPair {
        val params = V8Array(v8)
                .push(token)
        try {
            val decodedToken = v8aladinAndroid.executeStringFunction("wrapProfileToken", params)
    ...
    fun signProfileToken(profile: Profile, privateKey: String, subject: Entity, issuer: Entity, signingAlgorithm: SigningAlgorithm = SigningAlgorithm.ES256K, issuedAt: Date = Date(), expiresAt: Date = nextYear()): ProfileTokenPair {
        ...
        try {
            val signedToken = v8aladinAndroid.executeStringFunction("signProfileToken", params)
    ...
    fun verifyProfileToken(token: String, publicKeyOrAddress: String): ProfileToken {
        ...
        try {
            val decodedToken = v8aladinAndroid.executeStringFunction("verifyProfileToken", params)
            return ProfileToken(JSONObject(decodedToken))
        } catch (e: Exception) {
            Log.d(TAG, e.toString(), e)
            throw e
    ...
    fun parseZoneFile(zoneFileContent: String): ZoneFile {
        ...
        try {
            val zoneFile = v8aladinAndroid.executeStringFunction("parseZoneFile", params)
    ...
    fun resolveZoneFileToProfile(zoneFileContent: String, publicKeyOrAddress: String? = null, callback: (Result<Profile>) -> Unit) {
        ...
        try {
            v8aladinAndroid.executeVoidFunction("resolveZoneFileToProfile", params)
        } catch (e: Exception) {
            Log.d(TAG, e.toString(), e)
            throw e
        } finally {
    ...
    fun getAppBucketUrl(gaiaHubUrl: String, appPrivateKey: String, callback: (Result<String>) -> Unit) {
        ...
        val v8params = V8Array(v8)
                .push(gaiaHubUrl)
                .push(appPrivateKey)
        v8aladinAndroid.executeVoidFunction("getAppBucketUrl", v8params)
        v8params.release()
    }
     ...
     *
     *@param path the path to the file to read
     *@param username The aladin ID of the user to look up
     *@param appOrigin The app origin
     *@param zoneFileLookupURL The URL to use for zonefile lookup. If false, this will use the aladin.js's getNameInfo function instead.
    ...
    fun getUserAppFileUrl(path: String, username: String, appOrigin: String, zoneFileLookupURL: String?, callback: (Result<String>) -> Unit) {
        ...
        v8aladinAndroid.executeVoidFunction("getUserAppFileUrl", v8params)
        v8params.release()
    }


    fun getPublicKeyFromPrivate(privateKey: String): String? {
        return v8.executeStringScript("aladin.getPublicKeyFromPrivate('${privateKey}')")
    }

    fun makeECPrivateKey(): String? {
        return v8.executeStringScript("aladin.makeECPrivateKey()")
    }

    fun publicKeyToAddress(publicKey: String): String? {
        return v8.executeStringScript("aladin.publicKeyToAddress('${publicKey}')")
    }
    ...
    @Suppress("unused")
    private class JSAndroidBridge(private val aladinSession: AladinSession, private val v8: V8, private val v8aladinAndroid: V8Object, private val v8userSessionAndroid: V8Object, private val httpClient: Call.Factory) {

        fun signInSuccess(userDataString: String) {
            val userData = JSONObject(userDataString)
            if (aladinSession.signInCallback != null) {
                aladinSession.signInCallback!!.invoke(Result(UserData(userData)))
                aladinSession.signInCallback = null
            }
        }

        fun signInFailure(error: String) {
            if (aladinSession.signInCallback != null) {
                aladinSession.signInCallback!!.invoke(Result(null, error))
                aladinSession.signInCallback = null
            }
        }
        ...
        fun validateProofsResult(proofs: String) {
            ...
            aladinSession.validateProofsCallback?.invoke(Result(proofList))
        }

        fun validateProofsFailure(error: String) {
            aladinSession.validateProofsCallback?.invoke(Result(null, error))
        }

        fun resolveZoneFileToProfileResult(profileString: String) {
            val profile = Profile(JSONObject(profileString))
            aladinSession.resolveZoneFileToProfileCallback?.invoke(Result(profile))
        }

        fun resolveZoneFileToProfileFailure(error: String) {
            aladinSession.resolveZoneFileToProfileCallback?.invoke(Result(null, error))
        }

        fun lookupProfileResult(username: String, userDataString: String) {
            val userData = JSONObject(userDataString)
            aladinSession.lookupProfileCallbacks[username]?.invoke(Result(Profile(userData)))
        }

        fun lookupProfileFailure(username: String, error: String) {
            aladinSession.lookupProfileCallbacks[username]?.invoke(Result(null, error))
        }
        ...
        fun getFileResult(content: String?, uniqueIdentifier: String, isBinary: Boolean) {
            if (isBinary) {
                ...
                aladinSession.getFileCallbacks[uniqueIdentifier]?.invoke(Result(binaryContent))
            } else {
                aladinSession.getFileCallbacks[uniqueIdentifier]?.invoke(Result(content))
            }
            aladinSession.getFileCallbacks.remove(uniqueIdentifier)
        }

        fun getFileFailure(error: String, uniqueIdentifier: String) {
            aladinSession.getFileCallbacks[uniqueIdentifier]?.invoke(Result(null, error))
            aladinSession.getFileCallbacks.remove(uniqueIdentifier)
        }

        fun putFileResult(readURL: String, uniqueIdentifier: String) {
            aladinSession.putFileCallbacks[uniqueIdentifier]?.invoke(Result(readURL))
            aladinSession.putFileCallbacks.remove(uniqueIdentifier)
        }

        fun putFileFailure(error: String, uniqueIdentifier: String) {
            aladinSession.putFileCallbacks[uniqueIdentifier]?.invoke(Result(null, error))
            aladinSession.putFileCallbacks.remove(uniqueIdentifier)
        }

        fun deleteFileResult(uniqueIdentifier: String) {
            aladinSession.deleteFileCallbacks[uniqueIdentifier]?.invoke(Result(null))
            aladinSession.deleteFileCallbacks.remove(uniqueIdentifier)
        }

        fun deleteFileFailure(error: String, uniqueIdentifier: String) {
            aladinSession.deleteFileCallbacks[uniqueIdentifier]?.invoke(Result(null, error))
            aladinSession.deleteFileCallbacks.remove(uniqueIdentifier)
        }

        fun getFileUrlResult(url: String?, uniqueIdentifier: String) {
            aladinSession.getFileUrlCallbacks[uniqueIdentifier]?.invoke(Result(url))
            aladinSession.getFileUrlCallbacks.remove(uniqueIdentifier)
        }

        fun getFileUrlFailure(error: String, uniqueIdentifier: String) {
            aladinSession.getFileUrlCallbacks[uniqueIdentifier]?.invoke(Result(null, error))
            aladinSession.getFileUrlCallbacks.remove(uniqueIdentifier)
        }

        fun getAppBucketUrlResult(url: String) {
            aladinSession.getAppBucketUrlCallback?.invoke(Result(url))
        }

        fun getAppBucketUrlFailure(error: String) {
            aladinSession.getAppBucketUrlCallback?.invoke(Result(null, error))
        }

        fun getUserAppFileUrlResult(url: String) {
            aladinSession.getUserAppFileUrlCallback?.invoke(Result(url))
        }
        ...
        fun listFilesResult(url: String) {
            val cont = aladinSession.listFilesCallback?.invoke(Result(url))
            val params = V8Array(v8)
            params.push(cont)
        ...
        fun listFilesFailure(error: String) {
            aladinSession.listFilesCallback?.invoke(Result(null, error))
        }

        fun listFilesCountResult(count: Int) {
            aladinSession.listFilesCountCallback?.invoke(Result(count))
        }

        fun listFilesCountFailure(error: String) {
            aladinSession.listFilesCountCallback?.invoke(Result(null, error))
        }

        fun getSessionData(): String {
            return aladinSession.sessionStore.sessionData.json.toString()
        }

        fun setSessionData(sessionData: String) {
            aladinSession.sessionStore.sessionData = SessionData(JSONObject(sessionData))
        }

        fun deleteSessionData() {
            return aladinSession.sessionStore.deleteSessionData()
        }

        fun fetchAndroid(url: String, optionsString: String, keyForFetchUrl: String) {

            println("JSAndroidBridge.fetchAndroid")
            aladinSession.executor.onNetworkThread {
                println("JSAndroidBridge.fetchAndroid : onNetworkThread")
                val options = JSONObject(optionsString)

                val request = buildRequest(url, options)

                try {
                    println("JSAndroidBridge.fetchAndroid: onNetworkThread : try{}")
                    val response = httpClient.newCall(request).execute()
                    println("JSAndroidBridge.fetchAndroid: onNetworkThread : try: {$response}")
                    aladinSession.executor.onV8Thread {
        ...
        private fun executeFetchResolve(response: Response, keyForFetchUrl: String) {
            try {
                println("JSAndroidBridge.executeFetchResolve")
                val r = response.toJSONString()

                val v8params = V8Array(v8).push(keyForFetchUrl).push(r)
                v8aladinAndroid.executeVoidFunction("fetchResolve", v8params)
                v8params.release()
        ...
        private fun executeFetchReject(e: Exception, keyForFetchUrl: String) {
            try {
                ...
                v8aladinAndroid.executeVoidFunction("executeFetchReject", v8params)
                v8params.release()
            } catch (e: Exception) {
        ...
        fun setLocation(location: String) {
            aladinSession.executor.onMainThread {
                ...
                if (shouldLaunchInCustomTabs) {
                    ...
                    builder.setToolbarColor(ContextCompat.getColor(it, R.color.org_aladin_purple_50_logos_types))
                    builder.setToolbarColor(ContextCompat.getColor(it, R.color.org_aladin_purple_85_lines))
...
/**
 * Standard executor for using aladin session in an activity.
 */
...
class AndroidScriptRepo(private val context: Context) : ScriptRepo {
    override fun globals() = context.resources.openRawResource(R.raw.org_aladin_globals).bufferedReader().use { it.readText() }
    override fun aladin() = context.resources.openRawResource(R.raw.org_aladin_aladin).bufferedReader().use { it.readText() }
    override fun base64() = context.resources.openRawResource(R.raw.org_aladin_base64).bufferedReader().use { it.readText() }
    override fun zoneFile() = context.resources.openRawResource(R.raw.org_aladin_zone_file).bufferedReader().use { it.readText() }
    override fun aladinAndroid() = context.resources.openRawResource(R.raw.org_aladin_android).bufferedReader().use { it.readText() }
}
